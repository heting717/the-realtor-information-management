...
public class CustomerAction extends ActionSupport implements ModelDriven<Customer>{
	...
	public String findByPage(){
		...
		// 拼接查询的条件
		//房屋面积
		double cust_area = customer.getCust_area();
		if(cust_area != 0 ){
			// 说明，客户的名称输入值了
			criteria.add(Restrictions.eq("cust_area", cust_area));
		}
		//地理位置
		String cust_address = customer.getCust_address();
		if(cust_address != null && !cust_address.trim().isEmpty()){
			// 说明，客户的名称输入值了
			criteria.add(Restrictions.like("cust_address", "%"+cust_address+"%"));
		}
		//价格范围
		int cust_low_price = customer.getCust_low_price();
		int cust_high_price = customer.getCust_high_price();
		if(cust_low_price!=0&&cust_high_price!=0){
			criteria.add(Restrictions.between("cust_low_price", cust_low_price, cust_high_price));
			criteria.add(Restrictions.between("cust_high_price", cust_low_price, cust_high_price));
		}
		//客户名称
